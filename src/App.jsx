...
export default function ArchitectAI() {
  ...
  async function implement() {
    ...
    try {
      // Step 1: create the GitHub repo (idempotent — 422 means already exists)
      const createRes = await api('/api/github/create-repo', 'POST', {
        repoName,
        description: `${projectName} — generated by ArchitectAI`,
        private: false,
      });
      if (createRes.error && !createRes.error.includes('already exists')) throw new Error(createRes.error);
      setImplLog(prev => [...prev, {
        type: 'start',
        message: createRes.repoUrl ? `Repo created: ${createRes.repoUrl}` : `Repo ${repoName} already exists — continuing`,
      }]);
